...
class BEVFusion(Base3DDetector):
    ...
    def extract_pts_feat(self, batch_inputs_dict) -> torch.Tensor:
        ...
        if points is None:
            raise RuntimeError('extract_pts_feat called but no points provided')
        with torch.autocast('cuda', enabled=False):
            points = [point.float() for point in points]
            feats, coords, sizes = self.voxelize(points)
            batch_size = coords[-1, 0] + 1
        if self.pts_middle_encoder is None:
            raise RuntimeError('pts_middle_encoder is None but extract_pts_feat called')
    ...
    def extract_feat(
        ...
        **kwargs,
        ...
        # Only extract point features if points are available
        ...
        # Only apply pts_backbone and pts_neck if they are not None
