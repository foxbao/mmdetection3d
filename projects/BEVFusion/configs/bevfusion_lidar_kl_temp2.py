"""BEVFusion LiDAR-only + Temporal Fusion (2 historical frames) on KL dataset.

Based on bevfusion_lidar_kl_base.py. Main differences:
  - KlDataset with num_adj_frames=2
  - LoadTemporalData in pipeline
  - TemporalBEVFuser between pts_neck and bbox_head
  - Smaller batch size to fit temporal overhead in GPU memory
"""
    ...
    bbox_head=dict(
        ...
        train_cfg=dict(
            ...
            assigner=dict(
                ...
                cls_cost=dict(
                    type='mmdet.FocalLossCost', gamma=2.0, alpha=0.25,
                    weight=0.15),
...
train_pipeline = [
    ...
    # Load historical frame points + ego-motion (temporal)
    dict(type='LoadTemporalData', load_dim=5, use_dim=4,
         min_time_diff=0.2, max_time_diff=1.2),
    dict(type='ObjectSample', db_sampler=db_sampler),
    dict(type='GlobalRotScaleTrans', scale_ratio_range=[0.9, 1.1],
...
test_pipeline = [
    dict(type='LoadPointsFromFile', coord_type='LIDAR',
         load_dim=5, use_dim=4, backend_args=backend_args),
    # Load historical frame points for temporal inference
    dict(type='LoadTemporalData', load_dim=5, use_dim=4,
         min_time_diff=0.2, max_time_diff=1.2),
    dict(type='PointsRangeFilter', point_cloud_range=point_cloud_range),
    dict(type='Pack3DDetInputs',
...
#                        DATALOADER
# ==================================================================
# Smaller batch due to temporal overhead (2 adj frames processed per sample)
...
param_scheduler = [
    ...
    dict(type='CosineAnnealingLR', T_max=12, eta_min=lr * 1e-2,
         begin=8, end=20, by_epoch=True, convert_to_iter_based=True),
    dict(type='CosineAnnealingMomentum', T_max=8, eta_min=0.85 / 0.95,
         begin=0, end=8, by_epoch=True, convert_to_iter_based=True),
    dict(type='CosineAnnealingMomentum', T_max=12, eta_min=1,
         begin=8, end=20, by_epoch=True, convert_to_iter_based=True),
]

train_cfg = dict(by_epoch=True, max_epochs=6, val_interval=10)
val_cfg = dict()
test_cfg = dict()
...
auto_scale_lr = dict(enable=False, base_batch_size=28)
