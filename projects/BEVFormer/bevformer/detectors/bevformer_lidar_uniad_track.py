...
from ..dense_heads.bev_detr_head import inverse_sigmoid
...
class LidarUniADTrack(BEVFormerLidarUniAD):
    ...
    def loss(self, batch_inputs_dict: dict, batch_data_samples: List,
        ...
        assert len(batch_data_samples) == 1, (
            'LidarUniADTrack currently requires batch_size=1.')
        ...
        for t in range(T):
            bev_embed, track_instances = self._forward_frame(
    ...
    def _forward_frame(self, batch_inputs_dict, batch_data_samples,
        ...
        return bev_embed, track_instances
    ...
    def predict(self, batch_inputs_dict: dict, batch_data_samples: List,
                **kwargs):
        assert len(batch_data_samples) == 1, (
            'LidarUniADTrack predict requires batch_size=1.')
        sample = batch_data_samples[0]
        ...
        self._fill_track_data_sample(sample, active)
    ...
    def _fill_track_data_sample(self, data_sample, active: Instances) -> None:
        ...
        inst.bboxes_3d = box_type(
            boxes[keep], box_dim=boxes.size(-1), origin=(0.5, 0.5, 0.5))
        inst.scores_3d = scores[keep]
        inst.labels_3d = labels[keep]
        inst.instance_id = active.obj_idxes[keep]
        data_sample.pred_track_instances_3d = inst


@MODELS.register_module()
class LidarUniADTrackMemory(LidarUniADTrack):
    """MemoryBank ablation of ``LidarUniADTrack``.
