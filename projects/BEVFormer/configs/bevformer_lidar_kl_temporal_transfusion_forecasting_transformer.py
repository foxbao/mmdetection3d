"""Temporal TransFusion forecasting (transformer): head on frozen BEV.

This is the higher-capacity TransFusion counterpart of
``bevformer_lidar_kl_temporal_centerhead_forecasting_transformer.py``. It
keeps the frozen TransFusion detector / temporal stack and swaps in
``TransformerForecastingHead`` so motion queries can attend to both other
instances and the full BEV grid.

Use this after the smaller MLP head if you need more motion capacity, or when
you specifically want per-instance transformer queries for later planning work.
"""
...
work_dir = (
    './work_dirs/'
    'bevformer_lidar_kl_temporal_transfusion_forecasting_transformer')
