"""LiDAR-only BEVFormer on KL_8 — Stage 1 skeleton config.
...
Stage 1 wires the standard mmdet3d LiDAR voxel stack to a CenterHead so
we can verify the detector trains end-to-end. Stages 3-4 will replace
the post-neck path with a vendored BEVFormer Temporal Self-Attention
...
custom_imports = dict(
    imports=['projects.BEVFormer.bevformer'],
...
model = dict(
    ...
    pts_bbox_head=dict(
        type='CenterHead',
        in_channels=512,
        # Single task with all KL classes — keeps the Stage 1 head minimal.
        # Stage 3 will replace this with a BEVFormer-style decoder head,
...
# Short schedule for the Stage 1 skeleton — replaced in Stage 4.
...
work_dir = './work_dirs/bevformer_lidar_kl_stage1'
